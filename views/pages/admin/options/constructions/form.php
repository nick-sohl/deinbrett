...
$action = $isEdit ? '/admin/options/constructions/' . (int)$item['id'] . '/update' : '/admin/options/constructions';
?>
<div class="max-w-2xl">
  ...
  <form method="post" action="<?= $action ?>" class="space-y-4 bg-white rounded-lg border border-neutral-200 p-6">
    ...
    <div class="grid grid-cols-3 gap-4">
      <div><label class="block text-sm font-medium mb-1">Aufpreis (CHF)</label><input name="price_add" type="number" step="0.01" value="<?= $val('price_add', 0) ?>" class="w-full rounded border border-neutral-300 px-3 py-2 text-sm"></div>
    ...
    <div class="flex gap-3 pt-3 border-t border-neutral-100">
      ...
      <a href="/admin/options/constructions" class="text-sm text-neutral-500 hover:text-neutral-900 self-center">Abbrechen</a>
    </div>
  </form>
</div>
